...
namespace pb
{
	...
	void PhysicsSystem::Initialize()
	{
		...
		m_conLis = std::make_unique<ContactListener>();
		m_world->SetContactListener(m_conLis.get());
	}
	...
	b2Body* PhysicsSystem::CreateBody(const Vector2& position, float angle, const RigidBodyData& data)
	{
		...
		bodyDef.angle = pb::DegToRad(angle);
		bodyDef.fixedRotation = data.constraint_angle;
